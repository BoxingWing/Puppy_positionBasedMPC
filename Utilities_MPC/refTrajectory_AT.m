classdef refTrajectory_AT< matlab.System
    ...
    methods(Access = protected)
        ...
        function [refSeqOut,refP,refSeq,sitaErr,sitaNow] = stepImpl(obj,vxL,vyL,omegaZ,surVN,sura,X_FB,disable)
            ...
            vxLOut=obj.vxFilt*vxL+(1-obj.vxFilt)*obj.vxLOutOld;
            vyLOut=obj.vyFilt*vyL+(1-obj.vyFilt)*obj.vyLOutOld;
            wzLOut=obj.wzFilt*omegaZ+(1-obj.wzFilt)*obj.wzLOutOld;
            ...
            %             surVN=[0;0;1];
            %             sura=[0;0;0];
            obj.sitaZOld=obj.sitaZOld+wzLOut*obj.dt;
            tmp=Rz(obj.sitaZOld)*[vxLOut;vyLOut;0];
...
%             end
            ...
            for i=1:1:obj.numP
                Rbody=Rodrigues(surVN*wzLOut*obj.dt*(i-1))*RwOri;
                desTheta=Rot2Eul(Rbody);
                desTheta(3)=obj.sitaZOld+wzLOut*obj.dt*(i-1);
                if i<1.5
                    desr=[obj.pxOld;obj.pyOld;obj.r0(3)];
                else
                    desr=Rbody*[vxLOut;vyLOut;0]*obj.dt+refSeq(i-1,1:3)';
                end
                desdr=Rbody*[vxLOut;vyLOut;0];
                desdtheta=Rbody*[0;0;wzLOut];
                pSur=[0;0;sura(1)]; % get the point on the surface whose x and y coordinates are zero
                h=(desr-pSur)'*surVN;
                desr=desr+(desH-h)*surVN; % guarantee that the distance between the height and the robot are the desired height
            ...
            obj.vxLOutOld=vxLOut;
            obj.vyLOutOld=vyLOut;
            obj.wzLOutOld=wzLOut;
